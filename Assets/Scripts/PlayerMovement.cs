...
a = gameObject.transform.Find(models[CharacterSelection.charnum - 1]).GetComponent<Animation>();
    ...
    if(inp1){
      ...
      if(Cam.transform.position.z - 9 > gameObject.transform.position.z || gameObject.transform.position.x > 8 || gameObject.transform.position.x < -8){ // game over
...
void OnCollisionEnter(Collision collision){ // game over
